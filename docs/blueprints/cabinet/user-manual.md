...
The best way to find a document file in _cabinet_ would be to search for a person first and then use the button _Focus_, which excludes all other persons and their documents from the search results. This view now represent a student's registry. The listed documents can be further limited by applying the _Document properties_ filters.
